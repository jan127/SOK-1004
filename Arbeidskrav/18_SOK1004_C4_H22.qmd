...
mtcars %>%
  ...
  theme_minimal()
```
Binwidht endrer bredden på histogrammene. Ut i fra hvor mange observasjoner du har, burde man variere denne for å få en best mulig, men også leselig illustrasjon.
...
df <- df %>%
  ...
  rename(var = statistikkvariabel) %>%
  rename(verdi = value)
#lager nytt datasett utifra ønskede observasjoner 
df_min20 <- df %>% 
  filter(sysselsatte != "10-19 sysselsatte") %>% #velger bort sysselsatte gruppen med mindre enn 20 ansatte
  filter(år >= 2015) %>% #velger år over 2015.
  filter(næring != "Alle næringer") #fjerner alle næringer da jeg ønsker å se på de ulike næringene.

#lager plot
df_min20 %>% #datsett
  ggplot(aes(x=verdi, fill=sysselsatte)) + #definerer akse og fill 
  geom_histogram(color="black", alpha=0.4, binwidth = 120) +  #histogram
  facet_wrap(~ factor (næring, (levels=c("Industri", "Tjenesteyting", "Andre næringer")))) + #tre paneler i ønsket rekkefølge
  theme_fivethirtyeight() + #tema
  theme(legend.text=element_text(size=6.5)) #legend font størrelse
```
...
#lager plot
df_min20 %>% #datsett
  ggplot(aes(x=verdi, fill=var)) + #definerer akse og fill 
  geom_histogram(color="black", alpha=0.4, binwidth = 120) +  #histogram
  facet_wrap(~ factor (næring, (levels=c("Industri", "Tjenesteyting", "Andre næringer")))) + #tre paneler i ønsket rekkefølge
  theme_fivethirtyeight() + #tema
  theme(legend.text=element_text(size=6.5)) #legend font størrelse
