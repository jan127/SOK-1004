...
#Rydder opp og laster inn nødvendige pakker
rm(list=ls())
library(tidyverse)
library(PxWebApiData)

#Laster inn data for BNP i angitt periode.
data <- ApiData("http://data.ssb.no/api/v0/no/table/09842", Tid = paste(1970:2021), ContentsCode = "BNP")

#lagrer data fra dataset 2.
df_chr <- data[[2]]

#Gjør tid fra chr til int og endrer navn på kolonnene. 
df <- df_chr %>% 
...
#lenke til data
link <- "http://data.ssb.no/api/v0/no/table/09842"

#lage datasett, samt endringer
df_natural <- link %>% ApiData(Tid = paste(2000:2021), ContentsCode = c("BNP","MEMOBNP"))
#velger datasett 2
df_prephase1 <- df_natural[[2]]
#endrer navn på kolonnene og gjør tid til heltall (int)
df_prephase2 <- df_prephase1 %>% 
  mutate(Tid=parse_integer(Tid)) %>%
  rename(var=ContentsCode, tid=Tid, verdi=value)
#Endrer på verdi til å vise BNP i tusener.
df <- df_prephase2 %>%
...
df %>%
  ...
  geom_line() +
  labs(title = "BNP per innbygger i Norge",
       subtitle = "Rød graf viser BNP og blå graf viser BNP med faste 2015 priser",
       x = "År",
       y = "BNP per innbygger i 1000") +
  guides(color = "none") +
  theme_fivethirtyeight() +
  theme(axis.title = element_text())
```

Figuren overnfor viser hvordan Norges BNP har økt fra år 2000 til 2021. Da den røde grafen viser landets BNP, mens den blå grafen viser BNP med faste 2015 priser. Vi ser på den rød grafen at prisen på hva som produseres og eksporteres fra Norge varierer en del. I dag bidrar Norges oljeeksport til store deler av landets inntekt, og da prisene til oljen svinger, svinger også landets BNP (rød graf).

Den blå grafen viser landets BNP med faste 2015 priser, altså at prisen på varene som produseres i Norge ville vært den samme i 2000, som i 2021. Da ser vi at landet også produserer mer en tidligere år.

Fordelen med å se på begge grafene, er at vi kan se om prisendring eller økt produksjon var begrunnelsen til økningen i BNP. (evt. nedgang i BNP.)
...
Koden flyttet alle variablene "MEMOBNP" fra å stå under "BNP" verdiene til ved siden av. Så når vi nå ser på datasettet "df_wide", ser vi at vi har en kolonne som viser verdiene for BNP, og en kolonne som viser verdiene for MEMOBNP.
...
#Legger til to nye variabler som viser endringer i verdi fra året før.
df_wide <- df_wide %>%
  mutate(DBNP = BNP - lag(BNP, n=1L)) %>%
  mutate(DMEMOBNP = MEMOBNP - lag(MEMOBNP, n=1L))

#endrer rekkefølge slik at DBNP følger etter LBNP.
df_wide <- df_wide %>%
...
#Endrer fra 6 variabler til en felles variabel kallt var, og  verdiene til verdi.
df_long <- df_wide %>%
...
#Lenke til datasett
link2 <- "http://data.ssb.no/api/v0/no/table/09842"

#Lagre datasett
df_natural2 <- link2 %>% ApiData(Tid = paste(1970:2021), ContentsCode = c("BNP","MEMOBNP"))

#velger liste 2
df_prephase4 <- df_natural2[[2]]

#endrer navn på kolonnene og gjør tid til heltall (int)
df_prephase5 <- df_prephase4 %>% 
  mutate(Tid=parse_integer(Tid)) %>%
  rename(var=ContentsCode, tid=Tid, verdi=value)

#Utvider "var" til BNP og MEMOBNP
df_prephase6 <- df_prephase5 %>% 
  pivot_wider(names_from = var, values_from = verdi)

#Legger til verdiene til variablene året før i nye kolonner.
df_prephase7 <- df_prephase6 %>%
  mutate(LBNP = lag(BNP,n=1L)) %>%
  mutate(LMEMOBNP = lag(MEMOBNP,n=1L)) %>%
  relocate(LBNP, .before = MEMOBNP)
 
#Legger til endringen i BNP og MEMOBNP i prosent, kallt EBNP og EMEMOBNP
df_prephase8 <- df_prephase7 %>% 
  mutate(EBNP = (100*(BNP - lag(BNP, n=1L))/(lag(BNP, n=1L)))) %>%
  mutate(EMEMOBNP = (100*(MEMOBNP - lag(MEMOBNP, n=1L))/(lag(MEMOBNP, n=1L)))) %>%
  relocate(EBNP, .before = MEMOBNP)

#Flytter alle variablene til en kolonne.
df_prephase9 <- df_prephase8 %>% 
  pivot_longer(c("BNP", "LBNP", "EBNP", "MEMOBNP", "LMEMOBNP", "EMEMOBNP"), names_to = "var", values_to = "verdi")

#Velger kun endringer i prosent.
df2 <- df_prephase9 %>% 
  filter(var == c("EBNP", "EMEMOBNP")) 
...
df2 %>%
  ...
  geom_point() + 
  geom_smooth() + 
  labs(title = "Utvikling i Norges BNP fra 1970 til 2021",
       subtitle = "Rød graf viser BNP og
Blå graf viser BNP med faste 2015 priser (reel BNP)",
       x = "År",
       y = "Økning i prosent") +
  guides(color = "none") +
  theme_fivethirtyeight() +
...
Grafen i koden ovenfor viser veksten i BNP og reel BNP i Norge fra 1970 til 2021. Som vi kan avlese fra noen av punktene, er veksten i noen år negativ. I BNP vil prisene på olje og gass påvirke veksten, dermed noen negative resultater, da prisen kan være lavere en tidligere år. Når vi ser på reel BNP (cyan), vil en negativ vekst si at Norge produserte mindre enn året før, da prisene ikke påvirker denne.
